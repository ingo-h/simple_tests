...
// Author: 2021-03-09 Ingo Höft <Ingo@Hoeft-online.de>, last modify 2022-01-04

// Compile with: /usr/bin/g++ -pedantic-errors -Wall -o sockproof.a sockproof.cpp
...
int mysocket(char* a_ipaddr, int a_port)
{
    ...
    if (inet_pton(AF_INET, a_ipaddr,
                  (in_addr_t *)&local_addr.sin_addr.s_addr) != 1) {
    ...
    std::cout << "mysocket:listening on socket and accepting... (finish with ^C)\n";
    ...
    if (no_fd <= 0) {
        std::cerr << "mysocket: error selecting incoming connection on a socket\n";
...
{
    if (argc < 3) {
        std::cerr << "Usage:   " << argv[0] << " <local_ip_address> <port>\n"
                     "Example: " << argv[0] << " 127.0.0.1 22649\n"
